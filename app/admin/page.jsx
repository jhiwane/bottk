...
import { 
  ...
  Video, Bold, Italic, Type, Palette, FileCode, CheckCircle, UploadCloud, Loader2, Lock, Menu, Rocket, ImagePlus, History, FolderOpen
} from 'lucide-react';
...
export default function AdminPanel() {
  ...
  const [assetTarget, setAssetTarget] = useState(null); // 'news_images', 'news_gallery', 'editor'
  ...
  const handleToolbarClick = (action) => {
    switch(action) {
      ...
      case 'color': 
        const col = prompt('Ketik warna (contoh: red, blue, green):', 'red');
  ...
  const handleGalleryUpload = async (e) => {
    const urls = await uploadToCloudinary(Array.from(e.target.files));
    if (urls.length > 0) setNewGalleryItem(prev => ({ ...prev, src: urls[0] })); // Hanya ambil 1 untuk input sementara
    e.target.value = '';
  };
  ...
  const addGalleryItem = () => {
    ...
    setNewGalleryItem({ group: '', type: 'image', src: '', caption: '' }); // reset
        ...
        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden animate-in zoom-in-95">
          ...
          <div className="p-4 overflow-y-auto flex-1 grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3">
            ...
            {assets.map((url, i) => {
               ...
               return (
                 <div key={i} onClick={() => handleSelectAsset(url)} className="aspect-square relative rounded-xl overflow-hidden cursor-pointer hover:ring-4 hover:ring-blue-500 transition-all shadow-sm">
      ...
      return (
        <div className="animate-in fade-in max-w-5xl">
          <div className="flex items-center gap-4 mb-6">
            <button onClick={() => setIsEditingNews(false)} className="w-10 h-10 md:w-12 md:h-12 bg-white rounded-full flex justify-center items-center border border-gray-200 shadow-sm"><X size={20}/></button>
          ...
          <form onSubmit={handleSaveNews} className="space-y-6 md:space-y-8 bg-white p-6 md:p-10 rounded-[2rem] border border-gray-100 shadow-sm">
            ...
            <div>
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 mb-2">
                ...
                <select onChange={applyTemplate} className="bg-orange-50 text-orange-600 text-xs font-bold px-3 py-1.5 rounded-lg border border-orange-200 outline-none cursor-pointer">
                  ...
                  <option value="kegiatan">Template Kegiatan</option>
                  <option value="pengumuman">Template Pengumuman</option>
                </select>
              </div>
              <input type="text" value={newsForm.title} onChange={e => setNewsForm({...newsForm, title: e.target.value})} required className="w-full px-5 py-3.5 rounded-2xl bg-gray-50 border border-gray-200 font-bold text-gray-900" placeholder="Contoh: Lomba Mewarnai" />
            </div>
            ...
            <div className="p-6 bg-blue-50/50 rounded-3xl border border-blue-100">
              <label className="block text-sm font-bold text-blue-900 mb-4 flex items-center gap-2"><FolderOpen size={18}/> Kelola Galeri Kategori (Opsional)</label>
              ...
              <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-200 mb-4 flex flex-col md:flex-row gap-3">
                ...
                <div className="flex-1 space-y-3">
                  ...
                  <input type="text" value={newGalleryItem.group} onChange={e => setNewGalleryItem({...newGalleryItem, group: e.target.value})} className="w-full px-4 py-2 rounded-xl bg-gray-50 border border-gray-200 text-sm font-bold" placeholder="Nama Folder Kategori (Misal: Porseni)" />
                  <div className="flex gap-2">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1 mb-2 p-1.5 bg-gray-50 rounded-xl border border-gray-200">
                ...
                <button type="button" onClick={() => handleToolbarClick('quote')} className="p-2 text-gray-700 hover:text-blue-600 bg-white shadow-sm rounded-lg border border-gray-200 w-10">""</button>
    ...
    <div className="animate-in fade-in max-w-4xl">
      ...
      <form onSubmit={saveNewCloud} className="bg-white p-6 md:p-8 rounded-[2rem] shadow-sm border border-gray-100 mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div><label className="block text-sm font-bold text-gray-700 mb-2">Cloud Name</label><input type="text" value={newCloud.name} onChange={e => setNewCloud({...newCloud, name: e.target.value})} className="w-full px-5 py-3.5 rounded-2xl bg-gray-50 border border-gray-200" required placeholder="Contoh: dpqzxxx" /></div>
          <div><label className="block text-sm font-bold text-gray-700 mb-2">Upload Preset (Unsigned)</label><input type="text" value={newCloud.preset} onChange={e => setNewCloud({...newCloud, preset: e.target.value})} className="w-full px-5 py-3.5 rounded-2xl bg-gray-50 border border-gray-200" required placeholder="Contoh: tk_upload" /></div>
        </div>
        <button type="submit" className="w-full bg-gray-900 text-white font-bold py-3.5 rounded-xl shadow-md flex justify-center items-center gap-2"><Plus size={20} /> Tambahkan Akun</button>
      </form>
      ...
      <div className="space-y-4">
        ...
        {cloudAccounts.map(c => (
          <div key={c._id} className={`p-5 rounded-2xl border flex flex-col md:flex-row md:items-center justify-between gap-4 transition-colors ${c.active ? 'bg-green-50 border-green-200 shadow-sm' : 'bg-white border-gray-200'}`}>
            <div>
              <h4 className="font-bold text-gray-900 text-lg flex items-center gap-2 mb-1">{c.name} {c.active && <span className="bg-green-500 text-white text-[10px] px-2 py-0.5 rounded-md font-bold tracking-wider">AKTIF DIGUNAKAN</span>}</h4>
              <p className="text-sm text-gray-500 font-medium">Preset: {c.preset}</p>
            </div>
            <div className="flex gap-2 w-full md:w-auto">
              {!c.active && <button onClick={() => activateCloud(c._id)} className="flex-1 md:flex-none bg-white text-gray-800 border border-gray-300 font-bold px-5 py-2.5 rounded-xl text-sm">Gunakan Ini</button>}
              <button onClick={() => deleteCloud(c._id)} className="w-12 h-11 bg-red-50 text-red-500 rounded-xl flex items-center justify-center"><Trash2 size={18} /></button>
            </div>
          </div>
        ))}
      </div>
