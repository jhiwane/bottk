...
export default function Page() {
  ...
  const playVideo = (url, title) => {
    ...
    if (ytId) openAppIframe(`https://www.youtube.com/embed/${ytId}?autoplay=1`, title);
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-10">
          {filteredData.length === 0 ? (
            ...
            filteredData.map((item, idx) => {
              ...
              if (typeof thumb === 'string' && thumb.includes('cloudinary')) thumb = thumb.replace('/upload/', '/upload/w_200,q_auto,f_auto/');
  ...
  const renderToolsModal = () => (
    <div className={`fixed inset-0 z-[2000] bg-white/95 backdrop-blur-xl transform transition-transform duration-300 flex flex-col ${activeView === 'tools' ? 'translate-y-0' : 'translate-y-full'}`}>
      ...
      <div className="p-4 grid grid-cols-2 md:grid-cols-4 gap-4 overflow-y-auto">
        {toolsData.length === 0 ? (
          <p className="col-span-2 md:col-span-4 text-center text-gray-500 mt-10 bg-white/50 p-6 rounded-2xl backdrop-blur-md">Belum ada tools tersedia.</p>
        ) : (
          toolsData.map((t, idx) => {
  ...
  const renderIframeModal = () => (
    <div className={`fixed inset-0 z-[11000] bg-white transform transition-transform duration-300 flex flex-col ${activeView === 'iframe' ? 'translate-y-0' : 'translate-y-full'}`}>
      <div className="sticky top-0 bg-white/95 backdrop-blur-md p-4 shadow-sm flex items-center gap-4 z-10 border-b">
        <button onClick={() => setActiveView('home')} className="p-2 hover:bg-gray-100 rounded-full"><ArrowLeft size={24} className="text-gray-700" /></button>
      ...
      <div className="flex-1 w-full relative bg-gray-50">
        {isLoadingIframe && <div className="absolute inset-0 flex justify-center items-center backdrop-blur-sm bg-white/50 z-20"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-orange-500"></div></div>}
    ...
    return (
      <div className={`fixed inset-0 z-[3000] bg-black/95 backdrop-blur-xl transform transition-transform duration-300 overflow-y-auto flex flex-col ${activeView === 'mediaViewer' ? 'scale-100 opacity-100' : 'scale-95 opacity-0 pointer-events-none'}`}>
        <div className="sticky top-0 left-0 right-0 bg-black/50 backdrop-blur-md z-[301] px-4 py-4 flex items-center border-b border-white/10">
          <button onClick={() => setActiveView('detailNews')} className="flex items-center gap-3 text-white hover:text-gray-300 transition-colors bg-white/10 px-4 py-2 rounded-full backdrop-blur-sm"><ArrowLeft size={20} /> Kembali</button>
        </div>
              ...
              if (item.type === 'image') {
                return (
                  <div key={i} className="w-full flex flex-col items-center relative">
                    <Image src={item.src} width={1200} height={800} className="w-full h-auto max-h-[85vh] object-contain rounded-xl shadow-2xl mb-2 cursor-zoom-in" onClick={() => { pushHistory(); setZoomImage(item.src); }} alt="Galeri" />
                ...
                if (vid) {
                  return (
                    <div key={i} className="w-full flex flex-col items-center">
                      ...
                      <a href={`https://www.youtube.com/watch?v=${vid}`} target="_blank" rel="noreferrer" className="text-xs bg-amber-500/20 text-amber-400 px-3 py-1.5 rounded-full font-bold hover:bg-amber-500/40 transition-colors mb-4 flex items-center gap-2"><Youtube size={14} /> Buka di App YouTube</a>
                    </div>
                  );
                }
              }
    ...
    >
      ...
      <div style={{ display: activeView === 'home' ? 'block' : 'none' }}>
        ...
        <header id="beranda" className="relative w-full h-[100dvh] overflow-hidden flex items-center justify-center text-center bg-gray-900">
          ...
          <div className="absolute bottom-0 left-0 w-full overflow-hidden leading-[0] z-20 pointer-events-none"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none" className="relative block w-[calc(146%_+_1.3px)] h-[80px] text-white/30 fill-current backdrop-blur-sm"><path d="M321.39,56.44c58-10.79,114.16-30.13,172-41.86,82.39-16.72,168.19-17.73,250.45-.39C823.78,31,906.67,72,985.66,92.83c70.05,18.48,146.53,26.09,214.34,3V0H0V27.35A600.21,600.21,0,0,0,321.39,56.44Z"></path></svg></div>
        </header>
          ...
          <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-16">
             <div className="lg:w-1/2 relative h-96 md:h-[500px] w-full">
               ...
               <div className="w-full h-full relative rotate-2 hover:rotate-0 transition-transform duration-500 flex items-center justify-center">
                 <Image src={profileImages[0] || "https://images.unsplash.com/photo-1588075592446-265fd1e6e761?q=80&w=2072&auto=format&fit=crop"} fill sizes="(max-width: 768px) 100vw, 50vw" className="absolute inset-0 w-full h-full object-cover drop-shadow-2xl border-[10px] border-white/80 rounded-[3rem] z-20 shadow-[0_20px_50px_rgba(0,0,0,0.2)]" alt="Profile"/>
               </div>
             </div>
