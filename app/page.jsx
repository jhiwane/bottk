...
// === HELPER: OPTIMASI GAMBAR CLOUDINARY (tajam tapi ringan) ===
const optCloud = (url, w = 900) => {
...
// === FALLBACK ===
const FALLBACK_THUMB = "https://files.catbox.moe/3tf995.png";
const HERO_DEFAULT = "https://images.unsplash.com/photo-1588075592446-265fd1e6e761?q=80&w=1600&auto=format&fit=crop";
...
<span class="text-xs text-gray-500 italic">jika video tidak bisa dibuka di web ada tulisan kecil dibawah buka app youtube.</span>`,
  images: ["https://images.unsplash.com/photo-1588075592446-265fd1e6e761?q=80&w=1600&auto=format&fit=crop"],
  gallery: [
    { group: 'foto_1', type: 'image', src: "https://images.unsplash.com/photo-1588075592446-265fd1e6e761?q=80&w=1600&auto=format&fit=crop", caption: "Dokumentasi Kelas" },
...
// FIX: Hero tidak pernah hitam lagi — ada gambar default saat API belum siap
const defaultHeroImages = [HERO_DEFAULT];

const defaultProfileImages = [
  "https://images.unsplash.com/photo-1588075592446-265fd1e6e761?q=80&w=1200&auto=format&fit=crop",
  "https://images.unsplash.com/photo-1503676260728-1c00da094a0b?q=80&w=1200&auto=format&fit=crop"
];

// FIX BUG: Deteksi semua jenis link YouTube (youtu.be, watch, shorts, embed, live)
...
const GLOBAL_CSS = `
  ...
  .article-content a[onclick*="openMediaViewer"] {
      ...
      padding: 0 !important; color: #dc2626 !important; font-weight: 700 !important;
      display: inline-flex !important; align-items: center !important; gap: .25rem !important; text-indent: 0 !important;
  }
  .article-content a[onclick*="openMediaViewer"]:hover { text-decoration: underline !important; }
  .article-content p:has(a[onclick*="openMediaViewer"]) { text-align: left !important; display: inline !important; margin: 0 !important; text-indent: 0 !important; }
...
export default function Page() {
  ...
  // FIX: Smart embed — youtube-nocookie + rel=0 + modestbranding (lebih bersih)
  const playVideo = (url, title) => {
    const ytId = getYouTubeId(url);
    if (ytId) {
      openAppIframe(`https://www.youtube-nocookie.com/embed/${ytId}?autoplay=1&rel=0&modestbranding=1`, title);
  ...
  useEffect(() => {
    ...
    const loadContent = async () => {
      try {
        ...
        if (!isMounted) return;
        setNewsData(defaultNews); setVideoData(defaultVideos);
      } finally {
        if (isMounted) setTimeout(() => setIsLoadingGlobal(false), 500);
      }
    };
    loadContent();
  ...
  useEffect(() => {
    ...
    } else setNewsSlideIndex(0);
    return () => clearInterval(newsInterval);
  }, [activeView, currentDetail]);

  // === FIX: SLIDE OTOMATIS GALERI (jeda saat disentuh/hover) ===
  useEffect(() => {
  ...
  // === FIX: SLIDE OTOMATIS VIDEO (jeda saat disentuh/hover) ===
  useEffect(() => {
    ...
    const t = setInterval(() => {
      if (paused) return;
      if (el.scrollLeft + el.clientWidth >= el.scrollWidth - 12) el.scrollTo({ left: 0, behavior: 'smooth' });
  ...
  useEffect(() => {
    document.body.style.overflow = (isSidebarOpen || activeView !== 'home' || zoomImage || infoModalOpen) ? 'hidden' : 'auto';
  ...
  useEffect(() => {
    const handlePopState = () => {
      ...
      else if (activeView !== 'home') setActiveView(activeView === 'mediaViewer' ? 'detailNews' : 'home');
    };
  ...
  const handleScroll = (ref, direction) => {
    if (ref.current) ref.current.scrollBy({ left: direction * ref.current.clientWidth * 0.8, behavior: 'smooth' });
  };
  ...
  const openWhatsApp = (e) => {
    ...
    if (e && e.target && e.target.id === 'waForm') {
      msg = `Halo Admin TK BAITURROHMAN, saya ingin mendaftarkan anak saya.\n\n📋 *FORMULIR PENDAFTARAN*\n--------------------------------\n👤 *Nama Anak:* ${e.target.namaAnak.value}\n🎂 *Umur:* ${e.target.umur.value} Tahun\n⚧ *Jenis Kelamin:* ${e.target.jk.value}\n👨‍👩‍👧 *Nama Ortu:* ${e.target.namaOrtu.value}\n🏠 *Alamat:* ${e.target.alamat.value}\n--------------------------------\nMohon info selanjutnya. Terima kasih.`;
    }
    window.open(`https://wa.me/${phone}?text=${encodeURIComponent(msg)}`, '_blank', 'noopener');
  };

  const logoTightGlowStyle = {
    filter: 'drop-shadow(0 0 2px #fff) drop-shadow(0 0 5px #fff) drop-shadow(0 0 10px rgba(255,255,255,.9))'
  };
    ...
    <nav className="fixed w-full z-50 top-0 py-4 px-4 md:px-8 transition-all duration-500">
      <div className="max-w-7xl mx-auto flex justify-between items-center bg-white/70 backdrop-blur-2xl rounded-3xl md:rounded-full shadow-[0_8px_30px_rgb(0,0,0,0.04)] px-5 py-3 md:px-6 border border-white/60">
        ...
        <div className="flex items-center gap-3">
          <a href="#daftar" className="hidden md:block bg-blue-600 text-white px-6 py-2.5 rounded-full font-bold hover:shadow-lg hover:shadow-blue-500/30 transition-all duration-300 hover:-translate-y-0.5 text-sm">Daftar Sekarang</a>
          <button onClick={() => { pushHistory(); setIsSidebarOpen(true); }} className="w-11 h-11 md:w-12 md:h-12 bg-gray-100 hover:bg-gray-200 text-gray-800 rounded-full flex items-center justify-center transition-all duration-300">
            <Menu size={22} />
          </button>
        </div>
      </div>
  ...
  const renderSidebar = () => (
    <>
      <div className={`fixed inset-0 bg-black/40 backdrop-blur-sm z-[9998] transition-opacity duration-500 ease-out ${isSidebarOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={() => setIsSidebarOpen(false)} />
      <div className={`fixed top-0 right-0 h-full w-80 max-w-[85vw] bg-white/95 backdrop-blur-3xl shadow-2xl z-[9999] transform transition-transform duration-500 ease-out ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full'} flex flex-col overflow-y-auto border-l border-white/60 rounded-l-[2rem]`}>
        <div className="p-8 pb-4 flex justify-between items-center shrink-0">
          ...
          <button onClick={() => setIsSidebarOpen(false)} className="w-10 h-10 bg-gray-100 hover:bg-gray-200 rounded-full flex items-center justify-center text-gray-600"><X size={20} /></button>
        </div>
        <div className="flex flex-col p-6 gap-3 overflow-y-auto">
          <a href="#beranda" onClick={() => setIsSidebarOpen(false)} className="font-bold text-gray-600 hover:text-blue-600 hover:bg-blue-50 p-3 rounded-2xl flex items-center gap-4"><div className="w-8 flex justify-center text-blue-500"><Home size={22} /></div> Beranda</a>
          <a href="#profil" onClick={() => setIsSidebarOpen(false)} className="font-bold text-gray-600 hover:text-blue-600 hover:bg-blue-50 p-3 rounded-2xl flex items-center gap-4"><div className="w-8 flex justify-center text-blue-500"><Building size={22} /></div> Profil Sekolah</a>
          <a href="#galeri" onClick={() => setIsSidebarOpen(false)} className="font-bold text-gray-600 hover:text-blue-600 hover:bg-blue-50 p-3 rounded-2xl flex items-center gap-4"><div className="w-8 flex justify-center text-blue-500"><ImageIcon size={22} /></div> Galeri & Berita</a>
          <a href="#video" onClick={() => setIsSidebarOpen(false)} className="font-bold text-gray-600 hover:text-blue-600 hover:bg-blue-50 p-3 rounded-2xl flex items-center gap-4"><div className="w-8 flex justify-center text-blue-500"><Youtube size={22} /></div> Video Kegiatan</a>
          <div className="h-px bg-gray-100 my-2"></div>
          <button onClick={() => { pushHistory(); setActiveView('listNews'); setIsSidebarOpen(false); }} className="text-left font-bold text-gray-600 hover:text-blue-600 hover:bg-blue-50 p-3 rounded-2xl flex items-center gap-4 w-full"><div className="w-8 flex justify-center text-blue-500"><Newspaper size={22} /></div> Daftar Berita</button>
          <button onClick={() => { pushHistory(); setActiveView('listVideo'); setIsSidebarOpen(false); }} className="text-left font-bold text-gray-600 hover:text-blue-600 hover:bg-blue-50 p-3 rounded-2xl flex items-center gap-4 w-full"><div className="w-8 flex justify-center text-blue-500"><Youtube size={22} /></div> Daftar Video</button>
          <button onClick={() => { pushHistory(); setActiveView('tools'); setIsSidebarOpen(false); }} className="text-left font-bold text-gray-600 hover:text-blue-600 hover:bg-blue-50 p-3 rounded-2xl flex items-center gap-4 w-full"><div className="w-8 flex justify-center text-blue-500"><Rocket size={22} /></div> Tools & Aplikasi</button>
          <div className="h-px bg-gray-100 my-2"></div>
          <a href="https://docs.google.com/forms/d/e/1FAIpQLSfdM7hAS0t6Pbt1Sb4B43flvSZ2pg8JWpdaVlP0y3lv1mV_xg/viewform?usp=publish-editor" target="_blank" rel="noreferrer" className="bg-gray-50 border border-gray-200 text-gray-700 text-center py-4 rounded-2xl font-bold hover:bg-gray-100 flex items-center justify-center gap-2 mt-2"><FileText size={20} className="text-blue-600" /> Formulir online</a>
          <a href="#daftar" onClick={() => setIsSidebarOpen(false)} className="bg-orange-500 text-white text-center py-4 rounded-2xl font-bold shadow-lg shadow-orange-500/30 hover:bg-orange-600 mt-2">Daftar Sekarang</a>
        </div>
      </div>
    </>
  );
    ...
    return (
      <div className={`fixed inset-0 z-[2000] bg-gray-50/90 backdrop-blur-2xl transform transition-all duration-500 ease-out flex flex-col ${activeView.startsWith('list') ? 'translate-y-0 opacity-100 pointer-events-auto' : 'translate-y-full opacity-0 pointer-events-none'}`}>
        <div className="sticky top-0 bg-white/80 backdrop-blur-xl p-4 shadow-sm flex items-center gap-4 z-10 border-b border-gray-200/50">
          <button onClick={() => { setActiveView('home'); setSearchQuery(''); }} className="w-12 h-12 bg-white hover:bg-gray-50 rounded-full shadow-sm border border-gray-100 flex justify-center items-center"><ArrowLeft size={22} className="text-gray-700" /></button>
          <input type="text" placeholder={`Cari ${isNews ? 'berita' : 'video'}...`} className="flex-1 bg-white p-4 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500/30 shadow-sm border border-gray-100 text-gray-700 font-medium" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
        </div>
        <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-10">
          {filteredData.length === 0 ? (
            <div className="flex flex-col items-center justify-center mt-20 opacity-50"><ImageIcon size={64} className="mb-4 text-gray-400" /><p className="text-center text-gray-500 font-bold text-lg">Tidak ada data ditemukan.</p></div>
          ) : filteredData.map((item, idx) => {
            const title = item.title || item.judul;
            const date = item.date || 'Video Kegiatan';
            const ytId = !isNews && item.url ? getYouTubeId(item.url) : null;
            let thumb = isNews && item.images && item.images.length > 0 ? item.images[0] : FALLBACK_THUMB;
            return (
              <div key={idx} onClick={() => { if (isNews) { if (item.type === 'html') window.location.href = item.fileUrl; else { pushHistory(); setCurrentDetail(item); setActiveView('detailNews'); } } else playVideo(item.url, title); }} className="flex gap-5 bg-white p-4 rounded-3xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] cursor-pointer hover:shadow-lg transition-all hover:-translate-y-1 border border-gray-100">
                <div className="relative w-28 h-28 flex-shrink-0 overflow-hidden rounded-2xl bg-gray-100">
                  {ytId ? <YtThumb id={ytId} alt={title} /> : <Image src={optCloud(thumb, 300)} fill sizes="112px" className="object-cover" alt="Thumbnail" unoptimized />}
                </div>
  ...
  const renderToolsModal = () => (
    <div className={`fixed inset-0 z-[2000] bg-gray-50/90 backdrop-blur-2xl transform transition-all duration-500 ease-out flex flex-col ${activeView === 'tools' ? 'translate-y-0 opacity-100 pointer-events-auto' : 'translate-y-full opacity-0 pointer-events-none'}`}>
      <div className="sticky top-0 bg-white/80 backdrop-blur-xl p-4 shadow-sm flex items-center gap-4 z-10 border-b border-gray-200/50">
        <button onClick={() => setActiveView('home')} className="w-12 h-12 bg-white hover:bg-gray-50 rounded-full shadow-sm border border-gray-100 flex justify-center items-center"><ArrowLeft size={22} className="text-gray-700" /></button>
      ...
      <div className="p-6 grid grid-cols-2 md:grid-cols-4 gap-4 overflow-y-auto">
        {toolsData.length === 0 ? <p className="col-span-2 md:col-span-4 text-center text-gray-500 mt-10 font-bold">Belum ada tools tersedia.</p> : toolsData.map((t, idx) => {
          const link = (t.type === 'html_code') ? `/api/render?id=${t._id}` : t.url;
          return (
            <div key={idx} onClick={() => { if (t.type === 'html_code' || t.type === 'link') window.location.href = link; }} className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col items-center justify-center cursor-pointer hover:shadow-lg hover:-translate-y-1 transition-all">
              <div className="w-16 h-16 bg-gradient-to-tr from-blue-100 to-blue-50 rounded-2xl flex items-center justify-center mb-4 text-blue-600 shadow-inner"><Rocket size={28} /></div>
  ...
  const renderIframeModal = () => (
    <div className={`fixed inset-0 z-[11000] bg-black transform transition-all duration-500 ease-out flex flex-col ${activeView === 'iframe' ? 'translate-y-0 opacity-100 pointer-events-auto' : 'translate-y-full opacity-0 pointer-events-none'}`}>
      <button onClick={() => setActiveView('home')} className="absolute top-4 left-4 w-12 h-12 bg-black/40 hover:bg-black/70 backdrop-blur-md rounded-full flex justify-center items-center z-[11001] shadow-lg border border-white/20"><ArrowLeft size={22} className="text-white" /></button>
      <div className="flex-1 w-full relative bg-black">
        {isLoadingIframe && <div className="absolute inset-0 flex justify-center items-center z-20"><div className="animate-spin rounded-full h-12 w-12 border-4 border-gray-600 border-t-white"></div></div>}
        {activeView === 'iframe' && <iframe src={iframeData.url} className="w-full h-full border-0 absolute inset-0 z-10 bg-black" onLoad={() => setIsLoadingIframe(false)} title={iframeData.title} allowFullScreen allow="autoplay; encrypted-media; picture-in-picture" />}
      </div>
    </div>
  );
        ...
        <header className="bg-white border-b border-gray-200 sticky top-0 z-50 shadow-sm">
          <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <button onClick={() => setActiveView('home')} className="text-gray-600 hover:text-red-600"><ArrowLeft size={24} /></button>
            ...
            <nav className="hidden md:flex space-x-6 font-bold text-sm uppercase"><span className="text-red-600">Pendidikan</span></nav>
          </div>
        </header>
        <div className="w-full bg-gray-200 h-1"><div className="bg-red-600 h-1 w-1/3"></div></div>
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            <div className="lg:w-2/3 bg-white p-6 md:p-10 shadow-sm rounded-xl">
              <div className="flex items-center text-xs mb-4 space-x-2 text-gray-500">
                <span className="cursor-pointer hover:underline" onClick={() => setActiveView('home')}>Home</span><span>/</span><span className="cursor-pointer hover:underline">Pendidikan</span><span>/</span><span className="text-gray-400">Kegiatan Sekolah</span>
              </div>
              <div className="text-sm mb-2 text-green-700 font-bold uppercase tracking-widest">Info Sekolah</div>
              <h1 className="text-3xl md:text-4xl font-extrabold leading-tight mb-6 text-gray-900 text-left">{currentDetail.title}</h1>
              <div className="flex items-center space-x-3 mb-8 pb-6 border-b border-gray-100 text-sm">
                <div className="w-10 h-10 bg-gray-200 rounded-full flex items-center justify-center font-bold text-gray-500"><ImageIcon size={20} /></div>
                <div><p className="font-bold text-gray-900">Redaksi TK Baiturrohman</p><p className="text-gray-500 text-xs">{currentDetail.date}</p></div>
              </div>
                ...
                <figure className="mb-8 relative w-full max-w-lg mx-auto aspect-square bg-gray-100 rounded-[2rem] shadow-lg overflow-hidden group cursor-pointer" onClick={() => { if (currentDetail.images?.length) { pushHistory(); setZoomImage(currentDetail.images[newsSlideIndex]); } }}>
                  {currentDetail.images.map((src, i) => (
                    <div key={i} className={`absolute inset-0 w-full h-full transition-opacity duration-1000 ease-in-out ${i === newsSlideIndex ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}>
                      <Image src={optCloud(src, 1200)} fill sizes="(max-width:768px) 100vw, 720px" className="object-cover" alt="Hero" unoptimized />
                    </div>
                  ))}
              ...
              {currentDetail.gallery && currentDetail.gallery.length > 0 && (
                <div className="mt-12 pt-8 border-t border-gray-200">
                  ...
                  <div className="flex flex-col sm:flex-row justify-center gap-4">
                    <button onClick={() => { pushHistory(); setMediaViewerData(currentDetail); setActiveView('mediaViewer'); }} className="bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-6 rounded-xl shadow-md flex items-center justify-center gap-2 font-sans w-full md:w-auto"><ImageIcon size={20} /> Lihat Semua Dokumentasi</button>
                  </div>
                </div>
              )}
            ...
            <aside className="lg:w-1/3 space-y-8">
              <div className="bg-red-600 text-white p-8 rounded-xl text-center shadow-md">
                ...
                <button onClick={openWhatsApp} className="inline-block bg-white text-red-600 px-6 py-2 rounded-full font-bold text-sm uppercase shadow-lg hover:bg-gray-100">Info Selengkapnya</button>
              </div>
            </aside>
          </div>
        </main>
    ...
    return (
      <div className={`fixed inset-0 z-[3000] bg-black/95 backdrop-blur-2xl transform transition-transform duration-500 ease-out overflow-y-auto flex flex-col ${activeView === 'mediaViewer' ? 'scale-100 opacity-100' : 'scale-95 opacity-0 pointer-events-none'}`}>
        <div className="sticky top-0 left-0 right-0 z-[301] p-4 flex items-start bg-gradient-to-b from-black/80 to-transparent pointer-events-none">
          <button onClick={() => setActiveView('detailNews')} className="w-12 h-12 bg-white/10 hover:bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center text-white border border-white/10 shadow-lg pointer-events-auto"><ArrowLeft size={24} /></button>
        </div>
        <div className="flex-1 w-full max-w-4xl mx-auto p-4 flex flex-col gap-10 pb-20 items-center justify-center -mt-8">
          {filteredGallery.length === 0 ? <p className="text-white/50 font-medium">Tidak ada media.</p> : filteredGallery.map((item, i) => {
            if (item.type === 'image') {
              return (
                <div key={i} className="w-full flex flex-col items-center relative group">
                  <Image src={optCloud(item.src, 1400)} width={1200} height={800} className="w-full h-auto max-h-[85vh] object-contain rounded-2xl shadow-2xl mb-2 cursor-zoom-in" onClick={() => { pushHistory(); setZoomImage(item.src); }} alt="Galeri" unoptimized />
              ...
              return (
                <div key={i} className="w-full flex flex-col items-center">
                  <div className="w-full aspect-video rounded-2xl overflow-hidden shadow-2xl mb-2 bg-black border border-white/10">
                    <iframe className="w-full h-full" src={`https://www.youtube-nocookie.com/embed/${vid}?rel=0`} frameBorder="0" allowFullScreen title={item.caption || 'Video'}></iframe>
                  </div>
            ...
            return (
              <div key={i} className="w-full flex flex-col items-center">
                <div className="w-full aspect-video rounded-2xl overflow-hidden shadow-2xl mb-2 bg-black border border-white/10 flex justify-center items-center">
                  <video controls preload="metadata" className="w-full h-full object-contain" src={item.src}><source src={item.src} /></video>
                </div>
                {item.caption && <p className="text-gray-300 text-sm font-medium italic text-center mb-2 mt-2 bg-black/40 px-4 py-2 rounded-full">{item.caption}</p>}
              </div>
            );
    ...
    <div className="font-sans text-gray-800 min-h-screen relative overflow-x-hidden bg-[#f4f4f6]">
      ...
      {infoModalOpen && (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 animate-fade-in">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-md" onClick={() => setInfoModalOpen(false)}></div>
          <div className="relative bg-white/95 backdrop-blur-3xl rounded-[2.5rem] shadow-2xl w-full max-w-md p-8 border border-white overflow-hidden animate-zoom-in">
            ...
            <div className="relative z-10 text-center pt-6">
              <div className="w-20 h-20 bg-white rounded-3xl shadow-xl flex items-center justify-center mx-auto mb-6 text-blue-500 border border-gray-100 rotate-12"><Info size={36} className="-rotate-12" /></div>
              <h3 className="font-bold text-2xl text-gray-900 mb-3 tracking-tight">Info Terbaru</h3>
              <div className="bg-gray-50 p-5 rounded-2xl border border-gray-100 mb-8 text-left text-sm text-gray-600 font-medium">Selamat datang di sistem baru TK Baiturrohman. Informasi PPDB dan formulir pendaftaran kini bisa diakses dari menu!</div>
              <button onClick={() => setInfoModalOpen(false)} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 px-8 rounded-full shadow-lg shadow-blue-600/30 w-full">Tutup</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Zoom Modal */}
      {zoomImage && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-3xl z-[12000] flex flex-col justify-center items-center animate-fade-in" onClick={() => setZoomImage(null)}>
          <div className="fixed top-0 left-0 w-full p-6 flex justify-between z-[12001] bg-gradient-to-b from-black/80 to-transparent">
            ...
            <button onClick={() => setZoomImage(null)} className="w-12 h-12 bg-white/10 backdrop-blur-md rounded-full flex items-center justify-center text-white hover:bg-white/30"><X size={24} /></button>
          </div>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={zoomImage} className="max-w-[100vw] max-h-[100vh] object-contain cursor-zoom-out drop-shadow-2xl" onClick={() => setZoomImage(null)} alt="Zoom" />
        </div>
      )}
      ...
      <div style={{ display: activeView === 'home' ? 'block' : 'none' }}>
        ...
        <header id="beranda" className="relative w-full h-[100dvh] overflow-hidden flex items-center justify-center text-center bg-gray-900 rounded-b-[3rem] shadow-xl">
          ...
          {heroImages.map((src, i) => (
            <div key={i} className={`absolute inset-0 transition-opacity duration-[2000ms] ease-in-out ${i === heroIndex ? 'opacity-100 z-0' : 'opacity-0 -z-10'}`}>
              <Image src={optCloud(src, 1600)} fill priority={i === 0} sizes="100vw" className="object-cover animate-kenburns" alt="Hero Background" unoptimized />
            </div>
          ))}
          <div className="relative z-20 max-w-4xl px-6 flex flex-col items-center justify-center h-full pt-32 md:pt-44 pb-32">
            <div className="animate-slide-up">
              <span className="inline-block py-2.5 px-6 rounded-full bg-white/10 backdrop-blur-xl border border-white/20 text-white font-bold text-xs md:text-sm mb-8 shadow-xl">✨ Pendaftaran Siswa Baru Telah Dibuka</span>
              ...
              <p className="text-base md:text-2xl text-white/90 mb-10 font-medium max-w-2xl mx-auto drop-shadow-md">TK Baiturrohman membentuk karakter anak yang cerdas, kreatif, berakhlak mulia, Bertaqwa, Dan Berguna.</p>
              <div className="flex justify-center w-full">
                <a href="#daftar" className="bg-orange-500 hover:bg-orange-600 text-white font-bold py-4 px-12 rounded-full text-lg shadow-lg shadow-orange-500/40 transition-all duration-300 hover:-translate-y-1 border border-white/10">Daftar Sekarang</a>
              </div>
            </div>
          </div>
        </header>
        ...
        <section className="relative -mt-12 z-30 px-4 md:px-6 overflow-hidden">
          <div className="max-w-6xl mx-auto bg-white/60 backdrop-blur-2xl rounded-[3rem] shadow-[0_20px_60px_rgba(0,0,0,0.05)] p-8 md:p-12 grid grid-cols-1 md:grid-cols-3 gap-8 border border-white">
            {[
              { icon: BookOpen, color: 'blue', title: 'Kurikulum Merdeka', desc: 'Pembelajaran berpusat pada minat anak.' },
              { icon: Heart, color: 'orange', title: 'Pendidikan Islam', desc: 'Penanaman nilai agama sejak dini.' },
              { icon: Shapes, color: 'blue', title: 'Fasilitas Lengkap', desc: 'Area bermain aman & edukatif.' },
            ].map((f, i) => (
              <div key={i} className="text-center group p-6 rounded-3xl hover:bg-white/50 transition-all duration-300 cursor-default animate-fade-up" style={{ animationDelay: `${i * 120}ms` }}>
                <div className={`w-20 h-20 ${f.color === 'blue' ? 'bg-blue-100 text-blue-600 group-hover:rotate-6' : 'bg-orange-100 text-orange-500 group-hover:-rotate-6'} rounded-3xl flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-all duration-500 shadow-sm`}><f.icon size={32} /></div>
                <h3 className="font-bold text-2xl mb-3 text-gray-900 tracking-tight">{f.title}</h3>
        ...
        <section id="profil" className="py-24 px-6 relative">
          <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-16">
            <div className="lg:w-1/2 relative h-[400px] md:h-[550px] w-full">
              ...
              <div className="w-full h-full relative group rounded-[3rem] overflow-hidden" style={{ perspective: '1000px' }}>
                {profileImages.map((src, i) => (
                  <div key={i} className={`absolute inset-0 transition-opacity duration-[1500ms] ease-in-out ${i === profileIndex ? 'opacity-100 z-20' : 'opacity-0 z-10'}`}>
                    <Image src={optCloud(src, 1200)} fill sizes="(max-width:768px) 100vw, 50vw" className="object-cover transition-transform duration-700 ease-out group-hover:scale-[1.02]" alt={`Profile ${i + 1}`} unoptimized />
                  </div>
                ))}
            ...
            <div className="lg:w-1/2">
              ...
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {['Pendidik Profesional', 'Lingkungan Aman', 'Seni Lukis', "Baca Tulis Al-Qur'an"].map((t) => (
                  <li key={t} className="flex items-center gap-4 bg-white p-4 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md hover:-translate-y-1 transition-all"><div className="bg-blue-50 p-2 rounded-xl text-blue-600"><CheckCircle size={20} /></div><span className="font-bold text-gray-800 text-sm">{t}</span></li>
                ))}
              </ul>
            </div>
          </div>
        ...
        <section id="galeri" className="py-24 px-4 md:px-6 relative bg-white border-y border-gray-100 shadow-[inset_0_10px_30px_rgba(0,0,0,0.02)]">
          <div className="max-w-7xl mx-auto mb-12 text-center">
            ...
            <p className="text-gray-500 font-medium">Geser, ketuk, atau biarkan bergeser otomatis.</p>
          </div>
          <div className="max-w-[90rem] mx-auto relative group">
            ...
            <div ref={galleryRef} className="flex overflow-x-auto gap-6 hide-scroll snap-x snap-mandatory scroll-smooth pb-10 px-4 md:px-8">
              {newsData.map((n, i) => {
                const img = n.images && n.images.length > 0 ? optCloud(n.images[0], 900) : FALLBACK_THUMB;
                ...
                return (
                  <div key={i} onClick={() => { if (isHtml) openAppIframe(n.fileUrl, n.title); else { pushHistory(); setCurrentDetail(n); setActiveView('detailNews'); } }} className="w-[85vw] md:w-[400px] flex-shrink-0 snap-center rounded-[2.5rem] bg-white shadow-[0_8px_30px_rgba(0,0,0,0.06)] border border-gray-100 cursor-pointer group/item transition-all duration-500 ease-out hover:-translate-y-2 hover:shadow-[0_20px_40px_rgba(0,0,0,0.1)] overflow-hidden">
                    <div className="w-full aspect-square relative overflow-hidden bg-gray-100">
                      <Image src={img} fill sizes="(max-width:768px) 85vw, 400px" className="object-cover transition-transform duration-700 ease-out group-hover/item:scale-105" alt="News" unoptimized />
        ...
        {/* VIDEO — SLIDE OTOMATIS, THUMBNAIL JERNIH */}
        <section id="video" className="py-24 px-4 md:px-6 relative">
          ...
          <div className="max-w-[90rem] mx-auto relative group">
            <button onClick={() => handleScroll(videoRef, -1)} className="hidden md:flex absolute -left-6 top-1/2 -translate-y-1/2 z-20 w-14 h-14 rounded-full bg-white text-gray-800 shadow-[0_8px_30px_rgba(0,0,0,0.1)] border border-gray-100 items-center justify-center opacity-0 group-hover:opacity-100 transition-all hover:scale-110 hover:text-blue-600"><ChevronLeft size={28} /></button>
            <button onClick={() => handleScroll(videoRef, 1)} className="hidden md:flex absolute -right-6 top-1/2 -translate-y-1/2 z-20 w-14 h-14 rounded-full bg-white text-gray-800 shadow-[0_8px_30px_rgba(0,0,0,0.1)] border border-gray-100 items-center justify-center opacity-0 group-hover:opacity-100 transition-all hover:scale-110 hover:text-blue-600"><ChevronRight size={28} /></button>
                ...
                return (
                  <div key={i} className="w-[85vw] sm:w-[380px] md:w-[450px] flex-shrink-0 snap-center rounded-[2.5rem] bg-white shadow-[0_8px_30px_rgba(0,0,0,0.06)] border border-gray-100 group/vid transition-all duration-500 ease-out hover:-translate-y-2 hover:shadow-[0_20px_40px_rgba(0,0,0,0.1)] overflow-hidden">
                    ...
                    <div className="p-8">
                      ...
                      <button onClick={() => playVideo(v.url, v.judul)} className="w-full text-sm bg-gray-50 text-gray-700 font-bold hover:bg-gray-100 hover:text-blue-600 px-4 py-3 rounded-xl flex items-center justify-center gap-2 border border-gray-200"><Youtube size={18} className="text-red-500" /> Putar di Web</button>
                    </div>
                  </div>
                );
              })}
            ...
            <div className="md:w-5/12 bg-blue-600 p-10 md:p-16 text-white flex flex-col justify-between relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <div className="space-y-6 text-sm md:text-base">
                  <div className="flex items-center gap-4 bg-white/10 p-4 rounded-2xl backdrop-blur-md border border-white/10"><Phone size={24} className="text-blue-200 shrink-0" /><span className="font-bold tracking-wider">0895-3910-01402</span></div>
                  <div className="flex items-start gap-4 bg-white/10 p-4 rounded-2xl backdrop-blur-md border border-white/10"><MapPin size={24} className="mt-1 text-blue-200 shrink-0" /><span className="font-medium leading-relaxed">Jl. Andong kencono No. III, Pulodarat RT 19 RW 02, Pecangaan, Jepara.</span></div>
                </div>
              </div>
            </div>
            <div className="md:w-7/12 p-8 md:p-16 bg-white">
              <form id="waForm" className="space-y-6" onSubmit={openWhatsApp}>
                <div><label className="block text-sm font-bold text-gray-700 mb-2">Nama Lengkap Anak</label><input type="text" name="namaAnak" required className="w-full px-5 py-4 rounded-2xl bg-gray-50 border border-gray-200 focus:bg-white outline-none focus:ring-2 focus:ring-blue-500/30 text-gray-800 font-medium" placeholder="Contoh: Ahmad Zaky" /></div>
                <div className="grid grid-cols-2 gap-6">
                  <div><label className="block text-sm font-bold text-gray-700 mb-2">Umur (Tahun)</label><input type="number" name="umur" required className="w-full px-5 py-4 rounded-2xl bg-gray-50 border border-gray-200 focus:bg-white outline-none focus:ring-2 focus:ring-blue-500/30 text-gray-800 font-medium" placeholder="5" /></div>
                  <div><label className="block text-sm font-bold text-gray-700 mb-2">Jenis Kelamin</label><select name="jk" className="w-full px-5 py-4 rounded-2xl bg-gray-50 border border-gray-200 focus:bg-white outline-none focus:ring-2 focus:ring-blue-500/30 text-gray-800 font-medium cursor-pointer"><option>Laki-laki</option><option>Perempuan</option></select></div>
                </div>
                <div><label className="block text-sm font-bold text-gray-700 mb-2">Nama Orang Tua</label><input type="text" name="namaOrtu" required className="w-full px-5 py-4 rounded-2xl bg-gray-50 border border-gray-200 focus:bg-white outline-none focus:ring-2 focus:ring-blue-500/30 text-gray-800 font-medium" placeholder="Contoh: Bpk. Budi" /></div>
                <div><label className="block text-sm font-bold text-gray-700 mb-2">Alamat Domisili</label><textarea name="alamat" rows="3" required className="w-full px-5 py-4 rounded-2xl bg-gray-50 border border-gray-200 focus:bg-white outline-none focus:ring-2 focus:ring-blue-500/30 text-gray-800 font-medium resize-none" placeholder="Alamat lengkap..."></textarea></div>
                <button type="submit" className="w-full bg-gray-900 hover:bg-black text-white font-bold text-lg py-5 rounded-2xl shadow-xl shadow-gray-900/20 hover:-translate-y-1 flex justify-center items-center gap-3 transition-all duration-300">Kirim Pendaftaran</button>
              </form>
            </div>
          </div>
        </section>
          ...
          <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
            <div className="col-span-1 md:col-span-2">
              <div className="flex items-center gap-4 mb-6">
                <div style={logoTightGlowStyle}><Image src="/logotk.webp" width={64} height={64} className="h-16 w-auto object-contain" alt="Logo" /></div>
            ...
            <div>
              ...
              <ul className="space-y-4 text-gray-500 font-medium text-sm">
                ...
                <li><a href="#profil" className="hover:text-blue-600">Tentang Kami</a></li>
                <li><button onClick={() => { pushHistory(); setActiveView('listNews'); }} className="hover:text-blue-600">Galeri Foto</button></li>
                <li><a href="#daftar" className="hover:text-blue-600">Info PPDB</a></li>
              </ul>
            </div>
            <div>
